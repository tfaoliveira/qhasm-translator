...
sub search_structures
{
  ...
  for ($down = $down_cpy; $down < $up_cpy; $down++)
  {
    for ($up = $up_cpy; $up > $down; $up--)
    {
      ...
      if(!$found){ 
        $found = search_do_while_v2($lim_sup, $up, $down, $stack_ref, $tree_ref, $label_index_ref, 
                       $goto_index_ref, $inside_loop, $label_loop);
      }else{
        last;
      }
...
sub update_structures
{
  ...
  for (@$tree_ref)
  {
    ...
    if($_->{type} eq "dowhile_v2")
    {
      update_do_while_v2($_, $stack_ref, $trans_ref, $g_i_ref);
    }
...
sub update_do_while_v2
{
  my ($info, $stack_ref, $trans_ref, $g_i_ref) = @_;

  my $trans = $trans_ref->[$stack_ref->[$info->{end}]->{line}]->[0]; 
  ...
  if( $trans =~ m/if\((.+)\)\{/)
  {
    ...
    # GOTO IF
    $trans_ref->[$stack_ref->[$info->{end}]->{line}]->[0] = "}while($c);";
...
sub search_do_while_v2
{
  my ($lim_sup, $up, $down, $stack_ref, $tree_ref, $label_index_ref, $goto_index_ref, $inside_loop, $label_loop) = @_;

  if( ($up - $down) >= 2                                        &&
      $stack_ref->[$up]->{type}     eq "label"                  &&
      $stack_ref->[$down]->{type}   eq "gotoif"                 &&
      $stack_ref->[$down+1]->{type} eq "test"                   &&

      $stack_ref->[$up]->{arg1}     eq $stack_ref->[$down]->{arg1}  &&

      ($stack_ref->[$down+1]->{line} - $stack_ref->[$up]->{line})     >= 0)
  {
    return 0 if ! check_validity($up-1, $down+2, $stack_ref, $label_index_ref, $goto_index_ref, 1, $stack_ref->[$down]->{arg1}, 0, "");

    my (@childs_left, @childs_right) = ((),());
    my $r = {type => "dowhile_v2", begin => $up, end => $down, childs_left => \@childs_left, childs_rigth => \@childs_right};
    push @$tree_ref, $r;

    search_structures($lim_sup, $lim_sup, $up+1, $stack_ref, $tree_ref, $label_index_ref, $goto_index_ref, $inside_loop, $label_loop); # up
    search_structures($up-1, $up-1, $down+2, $stack_ref, \@childs_left, $label_index_ref, $goto_index_ref, 1, $stack_ref->[$down]->{arg1});  # inside
