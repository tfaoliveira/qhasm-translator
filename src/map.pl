...
sub print_function
{
	...
	my %ty = get_types_info("./config/types");
...
sub build_variable_types_intersection
{
	...
	for (@new)
	{
		$delete_ref->{$name}{$_} = 1 unless $_ ~~ @old;
	}

	for (@old)
	{
		if ($_ ~~ @new){ 
			$used_vars_types_ref->{$name}{$_} += $used_vars_ref->{$name}{$_};
		}else{
			$delete_ref->{$name}{$_} = 1;
		}
	}
...
sub find_mapping
{
	...
	for my $m (@$map_ref)
	{
		...
		if($cline =~ m/$rexp/)
		{
				my @mat   = ($1, $2, $3, $4);			# Possible matched variables or constants
...
sub calculate_let
{
	...
	my %siz = ('sizeof\(char\)' => 1, 'sizeof\(short\)' => 2, 'sizeof\(int\)' => 4);
