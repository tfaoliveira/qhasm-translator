...
sub parse_file
{
	...
	@file = grep {!/$CL_FINAL/} @file;												# Clean useless instructions
	map { s/^[ \t]*// } @file;															# Clean spaces and tabs at begin	
	...
	my $r = { arguments => { %$arguments_hash_ref }, declarations => { %$variables_hash_ref }, functions => [ @$functions_ref ] };
