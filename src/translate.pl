...
# Examples ##############################################################################
#															
# $ chmod +x translate.pl					make it executable
#													 
...
push @INC, "./src";  # This line adds the current folder in the environment variable include
...
if($print_parsed)
{
	...
	print "Arguments:\n";
	print "\t$t $n\n" while( ($n, $t) = each %{$parsed_file->{arguments}}    ); # Print arguments

	print "Variables:\n";
	print "\t$t $n\n" while( ($n, $t) = each %{$parsed_file->{declarations}} ); # Print other declarations
