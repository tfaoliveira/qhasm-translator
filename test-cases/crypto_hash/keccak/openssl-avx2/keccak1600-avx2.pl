...
#@A_jagged_print = map(8*($$_[0]*4+$$_[1]), @A_jagged_print);    # ... and now linear
#@A_jagged_print = map(($$_[0]*4+$$_[1]), @A_jagged_print);    # ... and now linear
#print join ',',@A_jagged_print;
#print "\n";
...
# inlined comments should be supported in translator
split_comments($code);
...
sub split_comments
{
  ...
  {
    #remove whitespace at beggining
    $line =~ s/^\s+//; $line =~ s/(\s+)/ /g;

    # lines that start with a comment
    if( $line =~ m/^\s*#/ )
    { push @nlines, $line; next; }

    # lines that contain code and comment
    if( $line =~ m/([^#]*)#(.*)/)
    {
      #push @nlines, "#$2"; # to remove comments comment this line
      push @nlines, $1;
      next; }
